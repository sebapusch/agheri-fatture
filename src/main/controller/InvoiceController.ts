...
const exchangeRateUri =
  "https://data-api.ecb.europa.eu/service/data/EXR/D.CHF.EUR.SP00.A?format=jsondata&startPeriod={startPeriod}";
...
export default class InvoiceController extends Controller {
  ...
  protected register() {
    ...
    this.app.handle("invoice.preview", async (invoice: any) =>
      this.preview(invoice)
    );
    this.app.handle("invoice.save", async (id: string) => this.save(id));
    this.app.handle("invoice.previewFromId", async (id: string) =>
      this.previewFromId(id)
    );
    ...
    this.app.handle("invoice.update", async (id: string, values: any) =>
  ...
  protected async update(id: string, values: any) {
    ...
    await service.bulkCreate(
      values.services.map((service: any) => ({ invoiceId: id, ...service }))
    );
    return updated[0] > 0;
  }
  ...
  private async prepare(invoice: any) {
    invoice.services = invoice.services.map((service: Service) => {
      if (
        [ServiceTypes.HOUR, ServiceTypes.LINE, ServiceTypes.WORD].includes(
          service.type
        )
      ) {
